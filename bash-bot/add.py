...
def add(ch1, ch2): #sums 2 chunks (ch2 is top)
	ch3 = [['' for x in range(16)] for x in range(8)]
	for linen in range(8):
		for charn in range(16):
			a = ch1[linen][charn]
			b = ch2[linen][charn]
			ch3[linen][charn] = addchar(ch1[linen][charn],ch2[linen][charn])
	return ch3
...
finsum = [['']*8 for x in range(maxy-miny)]
...
for x in range(maxx-minx):
	...
	for y in range(maxy-miny):
		...
		sum0 = add(lowch, topch)
		finsum[y-miny] = [finsum[y-miny][z]+''.join(sum0[z]) for z in range(8)]

print(''.join([''.join([sumc.replace('\\*','&').rstrip('&')+'\n' for sumc in sumr]) for sumr in finsum]))
